...
export function filterMuncher<
  InputTokenKind extends string = string,
  OutputTokenKind extends InputTokenKind = InputTokenKind,
>(
  ...
  return (state) => {
    ...
    while (
      match && !tokenKindWhiteList.includes(match.kind as OutputTokenKind)
    ) {
