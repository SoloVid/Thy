...
export default function CodeInput({
  ...
  function onKeyDown(event: KeyboardEvent) {
    ...
    if (event.key === "Tab") {
      if (element.selectionStart === element.selectionEnd && !event.shiftKey) {
        ...
        const selectedLines = value.substring(
          indexOfLineStart,
          indexOfEndLineStart + endLine.length,
        )
        ...
        element.setSelectionRange(
          indexOfLineStart,
          indexOfLineStart + modifiedLines.length,
        )
      }
    }
    ...
    if (event.key === "Backspace") {
      const lineUpToHere = currentLineBeforeHere.substring(
        0,
        currentIndex - indexOfLineStart,
      )
