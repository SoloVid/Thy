...
export function splitThyStatements(
  ...
  function submitMultilineString(soFar: ReduceState): ReduceState {
    ...
    const combinedString = interpretThyMultilineString(
      soFar.inProgressMultilineString,
    )
    return {
      ...soFar,
      statements: [
    ...
    (soFar0, line, lineIndex) => {
      ...
      if (soFar.multilineCommentTag !== null) {
        return {
          ...
          statements: [...soFar.statements, []],
          multilineCommentTag:
            line === soFar.multilineCommentTag
              ? null
              : soFar.multilineCommentTag,
        }
      }
      if (lineLevelIndent.length === ourLevelIndent.length) {
        ...
        const parts: Atom[] = /^[A-Z]/.test(line.trimStart())
          ? []
          : splitLineParts(line).map((p) => ({
              text: p.text,
              lineIndex: startingLineIndex + lineIndex,
              columnIndex: p.index,
            }))
        const openMultilineStringPart = parts[parts.length - 1]
        const opensMultilineString =
          parts.length > 0 && isAtomLiterally(openMultilineStringPart, `"""`)
        ...
        if (isAtomLiterally(andPart, "and")) {
          ...
          if (!lastStatement) {
            throw makeInterpreterError(
              andPart,
              "No preceding statement for `and` to match",
            )
          }
