...
function splitLinePartsJustStrings(
  ...args: Parameters<typeof splitLineParts>
): readonly string[] {
...
test("splitLineParts() should split line into parts of complex formats", async () => {
  assert.deepStrictEqual(splitLinePartsJustStrings(`a.b.c -148.0 "himom"`), [
    `a.b.c`,
    `-148.0`,
    `"himom"`,
  ])
...
test("splitLineParts() should not split a string with spaces", async () => {
  assert.deepStrictEqual(splitLinePartsJustStrings(`f "hi mom"`), [
    `f`,
    `"hi mom"`,
  ])
})
...
test("splitLineParts() should discard types", async () => {
  assert.deepStrictEqual(
    splitLinePartsJustStrings(`foo A String MyType a b c`),
    [`foo`, `a`, `b`, `c`],
  )
})
