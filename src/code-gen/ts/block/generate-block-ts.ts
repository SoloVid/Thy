...
import {
  checkAndGenerateTypeInstanceTs,
  generateTypeInstanceTs,
} from "../generate-type-instance-ts"
...
export function generateBlockTs(
  ...
  const topLevelContextTypes: ContextType[] = [
    contextType.blockAllowingExport,
...
function getParameterSpec(
  ...
  for (const typeArg of node.call.typeArgs.slice(1)) {
    state.addError(
      nodeError(typeArg, `given cannot take more than 1 type argument`),
    )
  }
...
function getReturnTypeSpec(
  ...
  for (const arg of node.args.slice(1)) {
    state.addError(
      nodeError(arg, `type return cannot take more than 1 argument`),
    )
  }
...
function resolvePreStatementGenerator(
