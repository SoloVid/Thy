...
export const namespaceThyGenerator: GeneratorForGlobalSpec &
  GeneratorForGlobalParentSpec = {
  name: "thy",
  ...
  generateAssignment(node, state, fixture) {
    ...
    if (referencedName === null) {
      state.addError(
        nodeError(
          sourceNameArg,
          `thy can only accept a string literal as an argument`,
        ),
      )
    ...
    const flattenedVariablePart = [variablePart].flat(
      Infinity as 1,
    ) as GeneratedSnippet[]
