  ...
  generateCall(node, state, fixture) {
    if (state.isExpressionContext()) {
      state.addError(
        nodeError(
          node.func,
          "thy.scope must be called as an independent statement",
        ),
      )
    ...
    for (const arg of node.args.slice(2)) {
      state.addError(
        nodeError(arg, `thy.scope cannot take more than 2 arguments`),
      )
    }
    ...
    if (namespaceName === null) {
      state.addError(
        nodeError(shouldBeStringName, `Scope name must be a string literal`),
      )
      return fromComplicated(node, ["// bad scope"])
    }
    ...
    const childState = state.makeChild({
      increaseIndent: true,
      context: contextType.blockAllowingExport,
    })
    if (shouldBeBlock.type !== "block") {
      state.addError(
        nodeError(shouldBeBlock, `Scope body must be a block literal`),
      )
      if (shouldBeBlock.type === "atom") {
        generatedBody = fromToken(shouldBeBlock.token)
      } else {
        generatedBody = fromComplicated(shouldBeBlock, [
          space2,
          "(",
          fixture.generate(shouldBeBlock, childState),
          ")()",
        ])
      }
    } else {
      generatedBody = generateBlockLinesTs(
        shouldBeBlock,
        shouldBeBlock.ideas,
        childState,
        fixture,
