...
export function atomGeneratorTs(standardLibrary: LibraryGeneratorCollection) {
  return makeAtomTsGenerator([standardLibrary.atomGenerator])
}

export function makeAtomTsGenerator(
  specializations: CodeGeneratorFunc<Atom>[],
...
export function generateAtomTs(
  atom: Atom,
    ...
    if (atom.symbolTable.getSymbolInfo(atom.token.text) === null) {
      if (!state.implicitArguments) {
        state.addError(
          nodeError(
            atom,
            `${atom.token.text} is not defined locally and there is no target for implicit arguments in this scope`,
          ),
        )
      } else {
        ...
        return fromToken(
          atom.token,
          `${state.implicitArguments.variableName}.${atom.token.text}`,
        )
      }
    }
