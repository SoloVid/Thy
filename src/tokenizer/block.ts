...
import { matchMemberAccessOperator, matchTypeIdentifier, matchValueIdentifier } from "./identifier";
import { makeIndentMatchers } from "./indent";
import { matchAwait, matchVarDeclAssign, matchExport, matchGiven, matchConstDeclAssign, matchLet, matchPrivate, matchReturn, matchStatementContinuation, matchThat, matchNoDeclAssign, matchType } from "./keywords";
...
export function makeBlockTokenizer(source: string, errors: CompileError[]): Tokenizer {
  ...
  const innerTokenizer = makeGenericTokenizer(finders, state, errors, () => !state.hasMoreText())
