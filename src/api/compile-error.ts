...
export function convertFromInternalError(
  ...
  const locationForCodeFrame: CodeFrameSourceLocation = {
    start: {
      line: simplifiedLocation.start.line + 1,
      column: simplifiedLocation.start.column + 1,
    },
  }
  if (internalError.start.text !== "" || internalError.end.text !== "") {
    if (simplifiedLocation.end.line === simplifiedLocation.start.line) {
      ...
      locationForCodeFrame.end = {
        line: simplifiedLocation.end.line + 1,
        column: simplifiedLocation.end.column + 1,
      }
    } else if (simplifiedLocation.end.column > 0) {
      locationForCodeFrame.end = {
        line: simplifiedLocation.end.line + 1,
        column: simplifiedLocation.end.column,
      }
    } else {
      const sourceLines = source.split("\n")
      const previousLine = sourceLines[simplifiedLocation.end.line].trimEnd()
      locationForCodeFrame.end = {
        line: simplifiedLocation.end.line,
